    ...
    return (
        <div className="border-t border-gray-200 p-4 bg-white">
            ...
            <div className="mb-4 text-xs text-gray-500">
                <span className="font-medium">Compatibilidad:</span> {getCompatibilityLabel(background)}
            </div>

            {/* Opciones de export */}
            <div className="space-y-2">
                <div className="text-xs font-medium text-gray-700 mb-2">Exportar como:</div>
                <div className="grid grid-cols-2 gap-2">
                    {exportOptions.map((option) => (
                        <Button
                            key={option.format}
                            onClick={() => handleCopy(option.text, option.format)}
                            className={`px-3 py-2 text-xs font-medium rounded-lg transition-all duration-200 ${copiedFormat === option.format
                                    ? "bg-green-100 text-green-700 border border-green-300"
                                    : "bg-gray-100 text-gray-700 border border-gray-200 hover:bg-gray-200 active:scale-95"
                                }`}
                        >
                            {copiedFormat === option.format ? <span className="flex items-center gap-2">
